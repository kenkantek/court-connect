		...
		<div class="col-xs-12 col-md-6">
			...
			<a class="btn btn-primary pull-right btn-new-court" href="" @click.prevent="addExtra()"><i class="fa fa-plus-circle"></i> Add Extra</a>
					<table class="table table-bordered table-hover table-th" id="datatables">
						...
						<tbody>
							<tr v-for="extra in contractSelect.extras">
								<template  v-for="(keyEx,valueEx) in extra">
									<td>{{ keyEx }}</td>
								  <td >{{ valueEx }}</td>
								</template>
...
<script>
	export default {
			props:   ['clubSettingId','contractSelect','reloadContracts','ifContractEdit'],			
			data() {
							return {
									rates :[
				...
				methods: {
					...
					addExtra(){
						...
						this.contractSelect.extras.push({ key: value });
						this.keyExtra = null;
